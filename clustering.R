...
seurat <- FindClusters(seurat, resolution = res_list[i] , random.seed = seed.use, reduction = "PCA")
...
## -------- Run Phate Analysis (As they removed PHATE integration from base Seurat, need to first add it back in by installing patched verio-----

#devtools::install_github("scottgigante/seurat", ref="patch/add-PHATE-again")

seurat <- RunPHATE(object = seurat, assay = "spliced")
